...
ggplot(data=predictdata, aes(x = mgstr_c, y = predict_logppm,group=state,color=state))+geom_line()+labs(title="Predicted log(ppm) by State", x="mgstr",y="log(ppm)")
...
levels(mergedata$race_code)[levels(mergedata$race_code)==""] <- "O"
levels(mergedata$race_code)[levels(mergedata$race_code)=="P"] <- "O"
####EDA#####
CrossTable(mergedata$party_cd,prop.c=F,prop.chisq=F,format="SPSS",max.width=10)
CrossTable(mergedata$race_code,prop.c=F,prop.chisq=F,format="SPSS",max.width=10)
crosstable(mergedata,sex_code,by= party_cd)%>%as_flextable(keep_id=TRUE)

ggplot(data = mergedata) + 
  geom_boxplot(aes(x=county_desc, y=turnout_rate))
ggplot(data = mergedata) + 
  geom_boxplot(aes(x=race_code, y=turnout_rate))
ggplot(data = mergedata) + 
  geom_boxplot(aes(x=ethnic_code, y=turnout_rate))
ggplot(data = mergedata) + 
  geom_boxplot(aes(x=age, y=turnout_rate))
ggplot(data = mergedata) + 
  geom_boxplot(aes(x=party_cd, y=turnout_rate))
...
#####Model######
modebas1<-glmer(cbind(voters_total,reg_total-voters_total) ~party_cd+sex_code+sex_code:party_cd+ethnic_code+age+race_code+age:party_cd+(1|county_desc),family=binomial(link="logit"),data=mergedata,control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
modebas<-glm(cbind(voters_total,reg_total-voters_total) ~party_cd+sex_code+sex_code:party_cd+ethnic_code+age+race_code+age:party_cd+county_desc,family=binomial(link="logit"),data=mergedata)
anova(modebas1,modebas,test="Chisq")

modebas2<-glmer(cbind(voters_total,reg_total-voters_total) ~party_cd+sex_code+sex_code:party_cd+ethnic_code+age:race_code+race_code+age:party_cd+(1|county_desc),family=binomial(link="logit"),data=mergedata,control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
modebas3<-glmer(cbind(voters_total,reg_total-voters_total) ~party_cd+sex_code+sex_code:party_cd+ethnic_code+age+race_code+age:party_cd+(1|county_desc),family=binomial(link="logit"),data=mergedata,control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
anova(modebas2,modebas3,test="Chisq")
#Final model
modebas2<-glmer(cbind(voters_total,reg_total-voters_total) ~party_cd+sex_code+sex_code:party_cd+ethnic_code+age:race_code+race_code+age:party_cd+(1|county_desc),family=binomial(link="logit"),data=mergedata,control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
summary(modebas2)
dotplot(ranef(modebas2, condVar=TRUE))
mode<-summary(modebas2)

plot_model(modebas2,show.values = TRUE)
plot_model(modebas2,type="pred")
p <- plot_model(modebas2,type="re",show.values = TRUE,ri.nr = c(1,2))
p[[1]]
p[[2]]

###Visualizing###
predictdataNC <- data.frame(race_code=unique(mergedata$race_code),party_cd="REP",sex_code="F",age="Age 41 - 65",ethnic_code="NL",county_desc="ALEXANDER")
predictdataNC$odds <-exp(predict(modebas2,predictdataNC))
predictdataNC1 <- data.frame(race_code=unique(mergedata$race_code),party_cd="DEM",sex_code="F",age="Age 41 - 65",ethnic_code="NL",county_desc="ALEXANDER")
predictdataNC1$odds <-exp(predict(modebas2,predictdataNC1))
predictdataNC2 <- data.frame(race_code=unique(mergedata$race_code),party_cd="LIB",sex_code="F",age="Age 41 - 65",ethnic_code="NL",county_desc="ALEXANDER")
predictdataNC2$odds <-exp(predict(modebas2,predictdataNC2))
predictdataNC3 <- data.frame(race_code=unique(mergedata$race_code),party_cd="CST",sex_code="F",age="Age 41 - 65",ethnic_code="NL",county_desc="ALEXANDER")
predictdataNC3$odds <-exp(predict(modebas2,predictdataNC3))
predictdataNC4 <- data.frame(race_code=unique(mergedata$race_code),party_cd="UNA",sex_code="F",age="Age 41 - 65",ethnic_code="NL",county_desc="ALEXANDER")
predictdataNC4$odds <-exp(predict(modebas2,predictdataNC4))
predictdata <- bind_rows(list(predictdataNC,predictdataNC1,predictdataNC2,predictdataNC3,predictdataNC4))
ggplot(data=predictdata, aes(x = race_code, y = odds,color=party_cd))+geom_point()

predictdataNC <- data.frame(race_code=unique(mergedata$race_code),party_cd="REP",sex_code="F",age="Age 41 - 65",ethnic_code="NL",county_desc="ANSON")
predictdataNC$logodds <-exp(predict(modebas2,predictdataNC))
predictdataNC1 <- data.frame(race_code=unique(mergedata$race_code),party_cd="DEM",sex_code="F",age="Age 41 - 65",ethnic_code="NL",county_desc="ANSON")
predictdataNC1$logodds <-exp(predict(modebas2,predictdataNC1))
predictdataNC2 <- data.frame(race_code=unique(mergedata$race_code),party_cd="LIB",sex_code="F",age="Age 41 - 65",ethnic_code="NL",county_desc="ANSON")
predictdataNC2$logodds <-exp(predict(modebas2,predictdataNC2))
predictdataNC3 <- data.frame(race_code=unique(mergedata$race_code),party_cd="CST",sex_code="F",age="Age 41 - 65",ethnic_code="NL",county_desc="ANSON")
predictdataNC3$logodds <-exp(predict(modebas2,predictdataNC3))
predictdataNC4 <- data.frame(race_code=unique(mergedata$race_code),party_cd="UNA",sex_code="F",age="Age 41 - 65",ethnic_code="NL",county_desc="ANSON")
predictdataNC4$logodds <-exp(predict(modebas2,predictdataNC4))
predictdata <- bind_rows(list(predictdataNC,predictdataNC1,predictdataNC2,predictdataNC3,predictdataNC4))
ggplot(data=predictdata, aes(x = race_code, y = logodds,color=party_cd))+geom_point()